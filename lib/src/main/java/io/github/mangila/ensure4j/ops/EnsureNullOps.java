...
public enum EnsureNullOps {
  ...
  /**
   * Returns the given object if it is not null; otherwise, it evaluates and returns the result from
   * the supplied {@link Supplier}. If the fallbackSupplier is null or produces a null value, it
   * throws an {@link EnsureException}.
   *
   * @param <T> the type of the object being checked
   * @param object the object to check for non-nullity
   * @param fallbackSupplier the fallbackSupplier to provide an alternative object if {@code object}
   *     is null
   * @return the non-null {@code object}, or the value provided by the {@code fallbackSupplier} if
   *     {@code object} is null
   ...
   *     exception * @deprecated Use {@link #notNull(Object)} ()} instead.
   */
  @Deprecated(since = "3.0.2", forRemoval = true)
  public <T> T notNullOrElseThrow(T object, Supplier<RuntimeException> runtimeExceptionSupplier)
   ...
   *     {@link RuntimeException} to be thrown if {@code object} is null
   * @return the non-null object
   * @throws RuntimeException if {@code object} is null, with the exception derived from the {@code
   *     runtimeExceptionSupplier}
   */
  public <T> T notNull(T object, Supplier<RuntimeException> runtimeExceptionSupplier)
      throws RuntimeException {
    if (isNull(object)) {
