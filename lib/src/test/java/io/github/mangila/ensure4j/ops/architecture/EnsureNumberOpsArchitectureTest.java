...
class EnsureNumberOpsArchitectureTest implements ClazzTest, PublicMethodArchitectureTest {
      ...
      switch (methodName) {
        case "max" -> {
          ...
          ArchTestUtils.checkOrAddMethodNameViolationEvent(
              javaClass, events, methodName, maxCount, count);
        }
        case "min" -> {
          int minCount = 6;
          var count = countByName.get(methodName);
          ArchTestUtils.checkOrAddMethodNameViolationEvent(
              javaClass, events, methodName, minCount, count);
        }
        case "negative" -> {
          int negativeCount = 6;
          var count = countByName.get(methodName);
          ArchTestUtils.checkOrAddMethodNameViolationEvent(
              javaClass, events, methodName, negativeCount, count);
        }
        case "negativeWithZero" -> {
          int negativeWithZeroCount = 6;
          var count = countByName.get(methodName);
          ArchTestUtils.checkOrAddMethodNameViolationEvent(
              javaClass, events, methodName, negativeWithZeroCount, count);
        }
        case "positive" -> {
          int positiveCount = 6;
          var count = countByName.get(methodName);
          ArchTestUtils.checkOrAddMethodNameViolationEvent(
              javaClass, events, methodName, positiveCount, count);
        }
        case "positiveWithZero" -> {
          int positiveWithZeroCount = 6;
          var count = countByName.get(methodName);
          ArchTestUtils.checkOrAddMethodNameViolationEvent(
              javaClass, events, methodName, positiveWithZeroCount, count);
        }
