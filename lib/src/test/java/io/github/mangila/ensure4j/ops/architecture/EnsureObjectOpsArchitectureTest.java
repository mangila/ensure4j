...
class EnsureObjectOpsArchitectureTest implements ClazzTest, PublicMethodArchitectureTest {
    ...
    for (String methodName : publicMethodNames) {
      switch (methodName) {
        ...
        case "isEquals" -> {
          ...
          var count = countByName.get(methodName);
          ArchTestUtils.checkOrAddMethodNameViolationEvent(
              javaClass, events, methodName, isEqualsCount, count);
        }
        case "isInstanceOf" -> {
          int isInstanceOfCount = 3;
          var count = countByName.get(methodName);
          ArchTestUtils.checkOrAddMethodNameViolationEvent(
              javaClass, events, methodName, isInstanceOfCount, count);
        }
