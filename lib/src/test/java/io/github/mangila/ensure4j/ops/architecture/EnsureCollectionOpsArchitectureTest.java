...
class EnsureCollectionOpsArchitectureTest implements ClazzTest, PublicMethodArchitectureTest {
    ...
    for (String methodName : publicMethodNames) {
      switch (methodName) {
        ...
        case "notContainsNull" -> {
          ...
          var count = countByName.get(methodName);
          ArchTestUtils.checkOrAddMethodNameViolationEvent(
              javaClass, events, methodName, notContainsNullCount, count);
        }
        case "notContainsNullIterate" -> {
          int notContainsNullIterateCount = 3;
          var count = countByName.get(methodName);
          ArchTestUtils.checkOrAddMethodNameViolationEvent(
              javaClass, events, methodName, notContainsNullIterateCount, count);
        }
        case "containsElement" -> {
          int containsElementCount = 3;
          var count = countByName.get(methodName);
          ArchTestUtils.checkOrAddMethodNameViolationEvent(
              javaClass, events, methodName, containsElementCount, count);
        }
