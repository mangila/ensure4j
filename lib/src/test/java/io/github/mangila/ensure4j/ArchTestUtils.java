...
public class ArchTestUtils {
  ...
  public static @NonNull Map<String, Long> getCountByPublicMethodNames(JavaClass javaClass) {
    return getPublicMethodNames(javaClass).stream()
        .collect(Collectors.groupingBy(methodName -> methodName, Collectors.counting()));
  }

  public static void checkOrAddMethodNameViolationEvent(
      JavaClass javaClass,
    ...
    if (actualCount != expectedCount) {
      events.add(
          SimpleConditionEvent.violated(
              javaClass,
              "method name %s was expected be called %s times but was %s"
                  .formatted(methodName, expectedCount, actualCount)));
    }
  }

  public static void addMissingMethodViolationEvent(
      JavaClass javaClass, ConditionEvents events, String methodName) {
    events.add(
        SimpleConditionEvent.violated(javaClass, "missing method name %s".formatted(methodName)));
  }
}
